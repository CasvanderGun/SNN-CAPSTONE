...
def create_output_spike_count_map(avg_spike_counts: dict, amount_of_neurons_shown: int, vmax: int, title: str, path: str) -> None:
        ...
        data_to_plot = np.array([avg_spike_counts[digit] for digit in sorted(avg_spike_counts.keys())])
        plt.figure(figsize=(8, 8))
        plt.imshow(data_to_plot, cmap='viridis_r', interpolation='nearest', origin='lower', aspect=amount_of_neurons_shown/10, vmin=0, vmax=vmax)
        plt.xlim(0,amount_of_neurons_shown - 0.5)
        plt.yticks(np.arange(10))
        plt.yticks(np.arange(10))
        plt.colorbar()
        plt.xlabel('Hiddden Neuron Index')
